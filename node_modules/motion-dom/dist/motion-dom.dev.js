(function (global, factory) {
    ...
    */
    ...
    function asRGBA(color) {
        ...
        if (!type) {
            motionUtils.warning(false, `'${color}' is not an animatable color. Use the equivalent color code instead.`, "color-not-animatable");
            return false;
        }
        ...
        const generator = {
            ...
            retarget: (keyframes, newVelocity) => {
                ...
                s.velocity = ignoreVelocity
                    ? 0
                    : -motionUtils.millisecondsToSeconds(newVelocity);
    ...
    function keyframes({ duration = 300, keyframes: keyframeValues, times, ease = "easeInOut", }) {
        ...
        if (keyframeValues.length === 2 &&
            ...
            const mixer = origin === target
                ? undefined
                : (motionUtils.MotionGlobalConfig.mix || mix)(origin, target);
            return {
                calculatedDuration: duration,
                next: (t) => {
                    state.value = mixer
                        ? mixer(easingFunctions(duration > 0 ? motionUtils.clamp(0, 1, t / duration) : 1))
                        : target;
                    state.done = t >= duration;
                    return state;
                },
    ...
    class JSAnimation extends WithPromise {
        ...
        initAnimation() {
            ...
            const generator = generatorFactory(keyframes$1 === options.keyframes
                ? options
    ...
    function readOrigin(value, name, target) {
        if (typeof value === "string") {
            if (motionUtils.isNumericalString(value) || motionUtils.isZeroValueString(value)) {
    ...
    function canAnimate(keyframes, name, type, velocity) {
        ...
        if (!isOriginAnimatable || !isTargetAnimatable) {
            // Only build the message when we're going to warn
            isOriginAnimatable !== isTargetAnimatable &&
                motionUtils.warning(false, `You are trying to animate ${name} from "${originKeyframe}" to "${targetKeyframe}". "${isOriginAnimatable ? targetKeyframe : originKeyframe}" is not an animatable value.`, "value-not-animatable");
            return false;
        }
    ...
    class MotionValue {
        ...
        on(eventName, callback) {
            var _a;
            if (eventName === "change")
                return this.onChangeSubscribe(callback);
            return ((_a = this.events)[eventName] || (_a[eventName] = new motionUtils.SubscriptionManager())).add(callback);
        }
        onChangeSubscribe(callback) {
            ...
            if (!events.change && !this.changeSubscriber) {
                ...
                if (!events.change) {
                    events.change = new motionUtils.SubscriptionManager();
    ...
    function createArcPath({ strength = 0.5, peak = 0.5, direction, rotate = false, } = {}) {
        ...
        const createInterpolator = (from, to) => {
            ...
            const tangentDelta = rotationScale
                ? motionUtils.wrap(-180, 180, tangent1 - tangent0)
                : 0;
            return (t) => {
                const out = {
                ...
                if (rotationScale) {
                    ...
                    out.rotate = motionUtils.wrap(-180, 180, raw - baseline) * rotationScale;
                }
                return out;
            };
        };
    ...
    function makeNoneKeyframesAnimatable(unresolvedKeyframes, noneKeyframeIndexes, name) {
        ...
        if (animatableTemplate && name) {
            for (const noneIndex of noneKeyframeIndexes) {
                 ...
                 */
                if (unresolvedKeyframes[noneIndex] === animatableTemplate)
                    continue;
    ...
    class DOMKeyframesResolver extends KeyframeResolver {
        ...
        measureInitialState() {
            ...
            if (measureKeyframe !== undefined) {
                this.motionValue?.jump(measureKeyframe, false);
            }
        }
        measureEndState() {
    ...
    function addEffect(effect) {
        motionUtils.invariant(typeof effect.test === "function" && typeof effect.read === "function", "Effects passed to animate.addEffect() need test() and read().", "effect-missing-test");
        removeEffect(effect);
        effects.unshift(effect);
    }
    function removeEffect(effect) {
        motionUtils.removeItem(effects, effect);
    }
        ...
        return animateValues((key) => {
            ...
            if (!value) {
                ...
                if (!element) {
                    ...
                    motionUtils.invariant(initial !== undefined, `"${key}" can't be read from the animated subject. Provide [from, to] keyframes.`, "effect-unreadable-value");
                }
    ...
    function isHTMLElement(element) {
        return (motionUtils.isObject(element) &&
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            if (parsed !== (key.startsWith("scale") ? 1 : 0)) {
                transform +=
                    (transform && " ") +
                        (openers[key] ||
                            (openers[key] = (translateAlias$1[key] || key) + "(")) +
    ...
    const propEffect = /*@__PURE__*/ createEffect((subject, state, key, value) => {
        ...
        test: (subject) => motionUtils.isObject(subject),
        read: (subject, key) => {
    ...
    function startViewAnimation(builder) {
        ...
        const resolveGroupTiming = (name) => {
            ...
            transition.duration && (transition.duration = motionUtils.secondsToMilliseconds(transition.duration));
            const { delay = 0, duration, ease } = applyGeneratorOptions(transition);
            return { delay: motionUtils.secondsToMilliseconds(delay), duration, ease };
        };
        /**
                    ...
                    for (const key of definitionNames) {
                        ...
                        for (let [valueName, valueKeyframes] of Object.entries(keyframes)) {
                            ...
                            if (valueName === "x" || valueName === "y") {
                                motionUtils.warnOnce(false, `animateView() animates view-transition layers with CSS properties; the "${valueName}" shorthand has no effect - use transform, e.g. { transform: "translateX(40px)" }.`);
                                continue;
                            }
                            /**
                    ...
                    if (name.type.startsWith("group")) {
                        ...
                        timing = {
                            ...
                            duration: isMorphCrossfade && visualDuration !== undefined
                                ? motionUtils.secondsToMilliseconds(visualDuration)
    ...
    class ViewTransitionBuilder {
        constructor(update, options = {}) {
            ...
            this.notifyReject = motionUtils.noop;
            ...
            this.readyPromise.catch(motionUtils.noop);
            addToQueue(this);
        }
        /**
    ...
    class HTMLVisualElement extends DOMVisualElement {
        ...
        mount(instance) {
             ...
             */
            motionUtils.invariant(Boolean(instance.style), "motion.create() components must forward their ref to a HTML or SVG element", "custom-component-ref");
    ...
    exports.addSVGValue = addSVGValue;
    ...
    exports.animateValues = animateValues;
    ...
    exports.handOffElementState = handOffElementState;
    exports.has2DTranslate = has2DTranslate;
    exports.hasComplexValues = hasComplexValues;
    ...
    exports.px = px;
    exports.readSVGValue = readSVGValue;
    exports.readStyleValue = readStyleValue;
    ...
    exports.styleSubjectEffect = styleSubjectEffect;
    ...
    exports.svgSubjectEffect = svgSubjectEffect;
    exports.sync = sync;
